...
```bash
SCENE_NAME=train_3
export AI2THOR_ROOT=/225010231/mwl/EMAS/memory/database
export AI2THOR_CONFIG_PATH=/225010231/mwl/EMAS/memory/concept-graphs/conceptgraph/dataset/dataconfigs/ai2thor/ai2thor.yaml
export GSA_PATH=/225010231/mwl/EMAS/memory/Grounded-Segment-Anything
...
```
注：这个代码运行的过程中，对transformer旧版的API做了多处修改：
第一处：/EMAS/memory/Grounded-Segment-Anything/recognize-anything/ram/models/bert.py
第二处：/EMAS/memory/Grounded-Segment-Anything/recognize-anything/ram/models/bertwrapper.py
第三处：/EMAS/memory/Grounded-Segment-Anything/recognize-anything/ram/models/utils.py
...
```
改版说明：
我们在将原来的LLaVA接口改为了通用接口，您可以将LLaVA改为任何您想要使用的VLM
使用方法：
第一步： 在__init__.py中加入您使用的模型
第二步： 创建模型调用代码，比如qwen.py
第三步： 仿照llava.py编写调用代码，确保调用函数对得上，里面可以做一些无意义的操作
第四步： 调用，比如
python scenegraph/build_scenegraph_cfslam.py \
  --mode extract-node-captions \
  --cachedir ${AI2THOR_ROOT}/${SCENE_NAME}/sg_cache \
  --mapfile ${AI2THOR_ROOT}/${SCENE_NAME}/pcd_saves/${PKL_FILENAME} \
  --vlm_backend qwen \
  --vlm_model_path /225010231/mwl/EMAS/CKPT/Qwen2.5-VL-7B-Instruct
python scenegraph/build_scenegraph_cfslam.py \
  --mode refine-node-captions \
  --cachedir ${AI2THOR_ROOT}/${SCENE_NAME}/sg_cache \
  --mapfile ${AI2THOR_ROOT}/${SCENE_NAME}/pcd_saves/${PKL_FILENAME} \
...
python scripts/visualize_cfslam_results.py \
    --result_path ${AI2THOR_ROOT}/${SCENE_NAME}/sg_cache/map/scene_map_cfslam_pruned.pkl.gz \
    --edge_file ${AI2THOR_ROOT}/${SCENE_NAME}/sg_cache/cfslam_object_relations.json
```
