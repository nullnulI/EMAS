...
def main(cfg : DictConfig):
    cfg = process_cfg(cfg)
    return_in_memory_map = bool(cfg.get("return_in_memory_map", False))
    ...
    if cfg.save_pcd:
        ...
        with gzip.open(pcd_save_path, "wb") as f:
            pickle.dump(pre_postprocess_results, f)
        print(f"Saved full point cloud to {pcd_save_path}")

    if return_in_memory_map:
        pre_postprocess_results = None
    
    objects = filter_objects(cfg, objects)
    objects = merge_objects(cfg, objects)
    post_postprocess_results = {
        'objects': objects.to_serializable(),
        'bg_objects': None if bg_objects is None else bg_objects.to_serializable(),
        'cfg': cfg,
        'class_names': classes,
    ...
    if cfg.save_pcd:
        post_pcd_save_path = pcd_save_path[:-7] + "_post.pkl.gz"
        with gzip.open(post_pcd_save_path, "wb") as f:
            pickle.dump(post_postprocess_results, f)
        print(f"Saved full point cloud after post-processing to {post_pcd_save_path}")
    ...
    result = {
        "pre": pre_postprocess_results,
        "post": post_postprocess_results,
        "pre_path": pcd_save_path,
        "post_path": post_pcd_save_path,
    }
    if return_in_memory_map:
        for obj in objects:
            for feature_name in ("clip_ft", "text_ft"):
                feature = obj.get(feature_name)
                if isinstance(feature, torch.Tensor):
                    obj[feature_name] = feature.detach().cpu()
        result["scene_map"] = objects
        result["post"] = None
    return result
