...
def generate_obs_from_poses(
    ...
    for i in trange(len(sampled_poses)):
        ...
        agent_event = event.events[agent_id] if hasattr(event, "events") and event.events is not None else event
        ...
        depth = np.asarray(agent_event.depth_frame).copy()
        instance = np.asarray(agent_event.instance_segmentation_frame).copy()

        # Compute the agent and camera pose. They are different!
        agent_pose = get_agent_pose_from_event(agent_event)
    ...
    last_agent_event = last_event.events[agent_id] if hasattr(last_event, "events") and last_event.events is not None else last_event
    obj_meta = last_agent_event.metadata["objects"]
    ...
    color_to_object_id = getattr(last_agent_event, "color_to_object_id", getattr(last_event, "color_to_object_id", {}))
    with open(color_to_object_id_path, "wb") as f:
        pickle.dump(color_to_object_id, f)
        
    object_id_to_color = getattr(last_agent_event, "object_id_to_color", getattr(last_event, "object_id_to_color", {}))
