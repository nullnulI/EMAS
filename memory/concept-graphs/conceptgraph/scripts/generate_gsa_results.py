...
DEFAULT_GSA_PATH = Path(__file__).resolve().parents[3] / "Grounded-Segment-Anything"
GSA_PATH = Path(os.environ["GSA_PATH"]) if "GSA_PATH" in os.environ else DEFAULT_GSA_PATH
if not GSA_PATH.exists() and DEFAULT_GSA_PATH.exists():
    print(f"GSA_PATH does not exist: {GSA_PATH}. Falling back to {DEFAULT_GSA_PATH}.")
    GSA_PATH = DEFAULT_GSA_PATH
if not GSA_PATH.exists():
    raise ValueError(
        "Please set the GSA_PATH environment variable to the path of the GSA repo. "
        f"Tried: {GSA_PATH}"
    )
GSA_PATH = str(GSA_PATH)
