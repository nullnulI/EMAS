"""
Build a scene graph from the segment-based map and captions from a VLM backend.
"""
...
CAPTION_CACHE_FILENAME = "cfslam_node_captions.json"
LEGACY_CAPTION_CACHE_FILENAME = "cfslam_llava_captions.json"
...
def get_openai_client():
    ...
    if _openai_client is None:
        ...
        if not api_key:
            raise RuntimeError(
                "OPENAI_API_KEY is not set. Set it for the GPT-based refinement flow, or "
                "pass a local --vlm_model_path to run refine-node-captions with a local VLM."
            )
...
def create_chat_completion(model: str, messages: list[dict], timeout: int):
...
def get_chat_completion_content(chat_completion) -> str:
...
def should_use_local_refinement(args) -> bool:
    if args.vlm_model_path is not None:
        return True
    if args.vlm_backend == "qwen" and os.getenv("QWEN2_5_VL_MODEL_PATH") is not None:
        return True
    if args.vlm_backend == "llava" and os.getenv("LLAVA_CKPT_PATH") is not None:
        return True
    return False


def format_local_chat_message(role: str, content: str) -> dict:
    if role == "user":
        return {"role": role, "content": [{"type": "text", "text": content}]}
    return {"role": role, "content": content}


def run_local_refinement_chat(chat, prompt_messages: list[dict], user_content: str) -> str:
    chat.reset()

    if hasattr(chat, "messages"):
        chat.messages = [
            format_local_chat_message(msg["role"], msg["content"])
    ...
    flattened_prompt = []
    for msg in prompt_messages:
        flattened_prompt.append(f"{msg['role'].upper()}:\n{msg['content'].strip()}")
    flattened_prompt.append(f"USER:\n{user_content}")
    flattened_prompt.append("ASSISTANT:")
    return chat("\n\n".join(flattened_prompt))
...
class ProgramArgs:
    ...
    # Captioning backend configuration
    vlm_backend: Literal["llava", "qwen"] = "qwen"
    vlm_model_path: Union[str, None] = None
    vlm_conv_mode: str = "v0_mmtag"
    vlm_num_gpus: int = 1


def get_caption_cache_path(args) -> Path:
    return Path(args.cachedir) / CAPTION_CACHE_FILENAME


def get_legacy_caption_cache_path(args) -> Path:
    return Path(args.cachedir) / LEGACY_CAPTION_CACHE_FILENAME


def load_caption_cache(args) -> list[dict]:
    caption_path = get_caption_cache_path(args)
    if not caption_path.exists():
        legacy_path = get_legacy_caption_cache_path(args)
        caption_path = legacy_path

    with open(caption_path, "r") as f:
        return json.load(f)


def save_caption_cache(args, caption_dict_list: list[dict]) -> None:
    caption_path = get_caption_cache_path(args)
    legacy_path = get_legacy_caption_cache_path(args)

    for path in [caption_path, legacy_path]:
        with open(path, "w", encoding="utf-8") as f:
            json.dump(caption_dict_list, f, indent=4, sort_keys=False)

def load_scene_map(args, scene_map, loaded_data: Any | None = None):
    """
...
def load_scene_map_from_data(args, scene_map_data: Any | None = None):
    from conceptgraph.slam.slam_classes import MapObjectList

    scene_map = MapObjectList()
    load_scene_map(args, scene_map, loaded_data=scene_map_data)
    return scene_map
...
def extract_node_captions(args, scene_map=None, chat=None):
    # NOTE: args.mapfile is in cfslam format
    if scene_map is None:
        scene_map = load_scene_map_from_data(args)
    ...
    if chat is None:
        ...
        print(f"{args.vlm_backend} chat initialized...")
    query = "Describe the central object in the image."

    # Directories to save features and captions
    savedir_feat = Path(args.cachedir) / "cfslam_feat_vlm"
    savedir_feat.mkdir(exist_ok=True, parents=True)
    savedir_captions = Path(args.cachedir) / "cfslam_captions_vlm"
    savedir_captions.mkdir(exist_ok=True, parents=True)
    savedir_debug = Path(args.cachedir) / "cfslam_captions_vlm_debug"
    ...
    for idx_obj, obj in tqdm(enumerate(scene_map), total=len(scene_map)):
        ...
        for idx_det in tqdm(idx_most_conf):
            ...
            if _w * _h < 70 * 70:
                ...
                print("small object. Skipping VLM captioning...")
            ...
            outputs = chat.answer_image(image_crop_modified, query)
            console.print(f"[bold green]{args.vlm_backend}:[/bold green] " + outputs)
        ...
        # Keep the existing cache file shape even for text-only backends such as Qwen.
        torch.save(torch.empty(0), savedir_feat / f"{idx_obj}.pt")
    ...
    save_caption_cache(args, caption_dict_list)
...
def refine_node_captions(args, scene_map=None, captions: list[dict] | None = None, chat=None):
    # NOTE: args.mapfile is in cfslam format
    from conceptgraph.scenegraph.GPTPrompt import GPTPrompt

    # Load the captions for each segment
    if captions is None:
        captions = load_caption_cache(args)
    ...
    local_chat = chat
    if local_chat is None and should_use_local_refinement(args):
        from conceptgraph.vlm import build_vlm_chat

        local_chat = build_vlm_chat(
            backend=args.vlm_backend,
            model_path=args.vlm_model_path,
            conv_mode=args.vlm_conv_mode,
            num_gpus=args.vlm_num_gpus,
        )
        print(f"Using local {args.vlm_backend} backend for caption refinement.")
    ...
    for _i in trange(len(captions)):
        ...
        if local_chat is not None:
            chat_content = run_local_refinement_chat(local_chat, gpt_messages, preds)
        else:
            ...
            chat_content = get_chat_completion_content(chat_completion)
        ...
        if elapsed_time > TIMEOUT:
            ...
            response_dicts.append(_dict)
            unsucessful_responses += 1
            continue
        
        # count unsucessful responses
        if "invalid" in chat_content.strip("\n"):
        ...
        _dict["response"] = chat_content.strip("\n")
...
def normalize_refined_response(response: dict | str) -> dict:
    ...
    if isinstance(raw_model_response, str):
        try:
            response["response"] = parse_json_response(raw_model_response)
        except Exception:
            response["response"] = {
                "summary": f"VLM json reply failed: Here is the invalid response {raw_model_response}",
...
def load_refined_responses(args, scene_map) -> tuple[list[dict], list[int]]:
    response_dir = Path(args.cachedir) / "cfslam_gpt-4_responses"
    responses = []
    missing_indices = []

    for idx in range(len(scene_map)):
        response_path = response_dir / f"{idx}.json"
        if not response_path.exists():
            missing_indices.append(idx)
            continue
        with open(response_path, "r") as f:
            responses.append(normalize_refined_response(json.load(f)))

    return responses, missing_indices


def build_scenegraph(args, scene_map=None, refined_responses: list[dict] | None = None, chat=None):
    from conceptgraph.slam.slam_classes import MapObjectList
    ...
    if refined_responses is None:
        responses, also_indices_to_remove = load_refined_responses(args, scene_map)
    else:
        responses = [normalize_refined_response(response) for response in refined_responses]
        response_ids = {response["id"] for response in responses}
        also_indices_to_remove = [idx for idx in range(len(scene_map)) if idx not in response_ids]
    ...
    object_tags_by_id = {
        response["id"]: response["response"].get("object_tag", "invalid")
        for response in responses
    }
    ...
    indices_to_remove = [
        idx
        for idx, object_tag in object_tags_by_id.items()
        if object_tag.lower() in ["fail", "invalid"]
    ]
    ...
    for _idx, segmentidx in enumerate(segment_ids_to_retain):
        ...
        pruned_responses.append(responses_by_id[segmentidx])
        pruned_object_tags.append(object_tags_by_id[segmentidx])
    ...
    if len(labels) != 0:
        ...
        if local_chat is None and should_use_local_refinement(args):
            ...
            print(f"Using local {args.vlm_backend} backend for object relations.")
        ...
        if not (Path(args.cachedir) / "cfslam_object_relations.json").exists():
            ...
            for componentidx, component in enumerate(components):
                ...
                for u, v in zip(
                    ...
                    relation_queries.append(json.loads(input_json_str))
                    ...
                    start_time = time.time()
                    if local_chat is not None:
                        chat_content = run_local_refinement_chat(
                            local_chat,
                            [{"role": "system", "content": DEFAULT_PROMPT.strip()}],
                            input_json_str,
                        )
                    else:
                        chat_completion = create_chat_completion(
                    ...
                    if elapsed_time > TIMEOUT:
                        ...
                        output_dict["raw_response"] = chat_content
                    else:
                        try:
                            # Attempt to parse the output as a JSON
                            chat_output_json = parse_json_response(chat_content)
    ...
    for componentidx, component in enumerate(components):
        ...
        for u, v in zip(
            ...
            relation = relations[_idx].get("object_relation", "FAIL")
            if relation in {"a on b", "b on a", "a in b", "b in a"}:
                scenegraph_edges.append((segmentidx1, segmentidx2, relation))
            _idx += 1
    ...
    return {
        ...
        "responses": responses,
        "relations": relations,
        "edges": scenegraph_edges,
        "components": components,
    }


def generate_scenegraph_json(args, scene_map=None):
